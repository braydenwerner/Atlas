...
//   try {
//     const res = await fetch(
//       `https://finnhub.io/api/v1/stock/candle?symbol=${stock}&resolution=${res}&from=${from}&to=${to}&token=${process.env.REACT_APP_FINNHUB_API_KEY}`
//     )
//     data = await res.json().catch((err: string) => errors.push(err))
//   } catch (err) {
//     errors.push(err)
...
//   try {
//     const res = await fetch(
//       `https://finnhub.io/api/v1/crypto/candle?symbol=BINANCE:${symbol}USDT&resolution=${res}&from=${from}&to=${to}&=${process.env.REACT_APP_FINNHUB_API_KEY}`
//     )
//     data = await res.json().catch((err: string) => errors.push(err))
//   } catch (err) {
//     errors.push(err)
